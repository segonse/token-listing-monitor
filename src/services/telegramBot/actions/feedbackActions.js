...
function setupFeedbackActions(bot) {
  ...
  bot.bot.action(/view_feedback_(\d+)/, async (ctx) => {
    ...
    const userDbId = users[0].id;
    const feedback = await FeedbackService.getFeedbackDetail(feedbackId, userDbId);
    ...
    message += `📄 <b>内容：</b>\n${feedback.content}\n`;
    message += `🕐 <b>提交时间：</b>${new Date(feedback.created_at).toLocaleString('zh-CN')}\n`;
    
    if (feedback.updated_at && feedback.updated_at !== feedback.created_at) {
      message += `🔄 <b>更新时间：</b>${new Date(feedback.updated_at).toLocaleString('zh-CN')}\n`;
    }
  ...
  bot.bot.action("view_more_feedback", async (ctx) => {
    ...
    return ctx.answerCbQuery("请使用Web界面查看完整的反馈历史", { show_alert: true });
  });
}
